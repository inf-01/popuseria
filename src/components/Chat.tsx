            ...
            <div className="h-[450px] overflow-y-auto neu-light-inset rounded-[40px] p-6 mb-8 space-y-6 border border-white/20">
                ...
                {error && (
                    <div className="flex flex-col items-center justify-center p-6 text-center space-y-4">
                        <div className="p-4 rounded-[20px] bg-red-50 text-red-500 border border-red-200 text-sm font-bold shadow-inner">
                            ⚠️ Lo sentimos, hubo un problema de conexión con la IA.
                            <br />
                            <span className="text-[10px] opacity-60 font-mono">{(error as any).message || 'Error de red'}</span>
                        </div>
                        <button
                            onClick={() => reload()}
                            className="text-xs text-[#e06c4c] font-black uppercase tracking-widest underline underline-offset-4"
                        >
                            Reintentar Conexión
                        </button>
                    </div>
