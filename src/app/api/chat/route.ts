...
export async function POST(req: Request) {
    return new Response("El bot está vivo, pero estamos probando la conexión...");

    const { messages } = await req.json();

    const apiKey = process.env.GOOGLE_GENERATIVE_AI_API_KEY || '';

    // Inicialización manual y ultra-segura del proveedor
...
`;
    ...
    console.log("📝 Petición de chat iniciada...");

    try {
        if (!apiKey) {
            throw new Error("API_KEY_MISSING: No se encontró la llave de Google en el servidor.");
        }
        ...
        return new Response(JSON.stringify({
            ...
            stack: "Verificar API Key en Vercel"
        }), {
            status: 500,
